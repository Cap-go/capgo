...
import { retryOperation, timedQuery } from '~/utils/supabaseUtils'

const { t } = useI18n()
const displayStore = useDisplayStore()
const main = useMainStore()
const isLoading = ref(false)
const isAddingNewKey = ref(false)
const isRegeneratingKey = ref<Set<string>>(new Set())
const isDeletingKey = ref<Set<string>>(new Set())
const isChangingName = ref<Set<number>>(new Set())
...
// Cache for organization and app names with size limits (max 1000 entries)
const orgCache = ref(new LRUCache<string, string>(1000))
const appCache = ref(new LRUCache<string, string>(1000))
...
// Helper computed property to get organization name by ID from LRU cache
const getOrgName = computed(() => {
  return (orgId: string) => orgCache.value.get(orgId) || 'Unknown'
})

// Helper computed property to get app name by ID from LRU cache
...
async function fetchOrgAndAppNames() {
  ...
  // Fetch organization names in parallel with timeout
  if (uncachedOrgIds.length > 0) {
    const orgPromises = uncachedOrgIds.map(async (orgId) => {
      try {
        ...
        handleError(err, t('failed-to-fetch-org-name'), `orgId: ${orgId}`)
  ...
  // Fetch app names in parallel with timeout
  if (uncachedAppIds.length > 0) {
    const appPromises = uncachedAppIds.map(async (appId) => {
      try {
        ...
        handleError(err, t('failed-to-fetch-app-name'), `appId: ${appId}`)
...
async function getKeys(retry = true): Promise<void> {
  ...
  try {
    // Use timedQuery with 10 second timeout
    ...
    handleError(error, t('failed-to-fetch-api-keys'), 'getKeys')
  }
  finally {
...
async function addNewApiKey() {
  ...
  try {
    ...
    if (displayStore.dialogCheckbox) {
      displayStore.unsetDialogCheckbox()
      const { data: apps, error } = await timedQuery(() =>
        supabase
          .from('apps')
          .select('*')
          .in('owner_org', selectedOrganizations),
      )

      if (error) {
        handleError(error, t('cannot-get-apps'), 'addNewApiKey')
        return
      }
    ...
    if (error) {
      handleError(error, t('failed-to-add-api-key'), 'addNewApiKey')
      return
    }
    ...
    handleError(error, t('error-adding-api-key'), 'addNewApiKey')
  }
  finally {
    isAddingNewKey.value = false
  }
}
...
async function regenrateKey(app: Database['public']['Tables']['apikeys']['Row']) {
  ...
  isRegeneratingKey.value.add(app.key)
  try {
    const newApiKey = crypto.randomUUID()
    const { data: { user } } = await supabase.auth.getUser()

    if (!user) {
      handleError(new Error('User not authenticated'), t('not-logged-in'), 'regenrateKey')
      return
    }

    // Use timedQuery with retry for better reliability
    ...
    if (error || typeof newApiKey !== 'string') {
      handleError(error || new Error('Invalid API key format'), t('failed-to-regenerate-key'), 'regenrateKey')
      return
    }
    ...
    handleError(error, t('error-regenerating-key'), 'regenrateKey')
  }
  finally {
    isRegeneratingKey.value.delete(app.key)
  }
}
...
async function deleteKey(key: Database['public']['Tables']['apikeys']['Row']) {
  ...
  isDeletingKey.value.add(key.key)
  try {
    // Use timedQuery with timeout
    ...
    if (error) {
      handleError(error, t('failed-to-delete-key'), 'deleteKey')
      return
    }
    ...
    handleError(error, t('error-deleting-key'), 'deleteKey')
  }
  finally {
    isDeletingKey.value.delete(key.key)
  }
}
  ...
  displayStore.dialogOption = {
    ...
    buttons: [
      ...
      },
      {
        ...
        id: 'confirm-button',
        handler: async () => {
          isChangingName.value.add(key.id)
          try {
            ...
            if (error) {
              handleError(error, t('cannot-change-name'), 'changeName')
              return
            }
            ...
            handleError(error, t('error-changing-name'), 'changeName')
          }
          finally {
            isChangingName.value.delete(key.id)
          }
        },
      },
    ],
    ...
    <div class="w-full h-full px-4 py-8 mx-auto max-w-9xl lg:px-8 sm:px-6">
      <div class="flex flex-col">
        ...
        <div v-if="isLoading" class="flex justify-center items-center py-10">
          ...
          <span class="ml-3 text-gray-600 dark:text-gray-300">{{ t('loading-api-keys') }}</span>
        </div>
          ...
          <div class="flex flex-col overflow-hidden overflow-y-auto bg-white rounded-lg shadow-lg border-slate-300 md:mx-auto md:mt-5 md:w-2/3 md:border dark:border-slate-900 dark:bg-slate-800">
            <dl :key="magicVal" class="divide-y dark:divide-slate-500 divide-slate-200">
              <InfoRow v-for="key in truncatedKeys" :key="key.id" :label="key.name" :is-link="false">
                ...
                <button
                  class="mx-1 text-center bg-transparent rounded-sm w-7 h-7 hover:bg-slate-100 dark:hover:bg-slate-600 relative"
                  :disabled="isRegeneratingKey.has(key.key)"
                  @click="regenrateKey(key)"
                >
                  <Spinner v-if="isRegeneratingKey.has(key.key)" size="h-5 w-5" class="absolute inset-1" />
                  <ArrowPath v-else class="mx-auto text-lg" />
                </button>

                <button
                  class="mx-1 bg-transparent rounded-sm w-7 h-7 hover:bg-slate-100 dark:hover:bg-slate-600 relative"
                  :disabled="isChangingName.has(key.id)"
                  @click="changeName(key)"
                >
                  <Spinner v-if="isChangingName.has(key.id)" size="h-5 w-5" class="absolute inset-1" />
                  <Pencil v-else class="mx-auto text-lg" />
                </button>

                <button
                  class="mx-1 bg-transparent rounded-sm w-7 h-7 hover:bg-slate-100 dark:hover:bg-slate-600"
                  @click="copyKey(key)"
                >
                ...
                <button
                  class="mx-1 bg-transparent rounded-sm w-7 h-7 hover:bg-slate-100 dark:hover:bg-slate-600 relative"
                  :disabled="isDeletingKey.has(key.key)"
                  @click="deleteKey(key)"
                >
                  <Spinner v-if="isDeletingKey.has(key.key)" size="h-5 w-5" class="absolute inset-1" />
                  <Trash v-else class="mx-auto text-lg text-red-600" />
                </button>
              </InfoRow>
            </dl>
          </div>
      ...
      <button
        class="fixed z-20 bg-gray-800 btn btn-circle btn-xl btn-outline right-4-safe bottom-4-safe secondary relative"
        :disabled="isAddingNewKey"
        @click="addNewApiKey"
      >
        <Spinner v-if="isAddingNewKey" size="h-6 w-6" class="absolute inset-0 m-auto" />
        <plusOutline v-else />
      </button>
    </div>
  </div>

...
<template>
  <div class="flex flex-col col-span-full overflow-hidden bg-white rounded-lg border shadow-lg sm:col-span-6 xl:col-span-4 dark:bg-gray-800 border-slate-300 h-[460px] dark:border-slate-900">
    ...
    <div class="flex gap-2 justify-between items-start px-4 pt-4">
      ...
      <div v-if="total !== undefined" class="flex flex-col items-end text-right shrink-0 min-w-0">
        <!-- Evolution badge -->
        <div
        ...
        <!-- Total value -->
        <div class="text-2xl font-bold whitespace-nowrap dark:text-white text-slate-600 sm:text-3xl">
          {{ total?.toLocaleString() }}<span v-if="unit" class="text-xl font-normal sm:text-2xl"> {{ unit }}</span>
        </div>
      </div>
    </div>

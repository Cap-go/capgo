...
<template>
  ...
  <div v-else class="h-full pb-4 overflow-hidden">
    ...
    <button v-if="!stepsOpen && !isMobile" class="fixed z-40 bg-gray-800 btn btn-circle btn-xl btn-outline right-4-safe bottom-20-safe md:right-4-safe md:bottom-4-safe secondary" @click="stepsOpen = true">
      <plusOutline />
    </button>
  </div>
</template>

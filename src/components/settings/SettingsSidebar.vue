      ...
      <ul class="flex mr-3 flex-nowrap md:block md:mr-0">
        <router-link v-slot="{ href, navigate, isExactActive }" to="/dashboard/settings/account" custom>
          <li class="mr-0.5 md:mr-0 md:mb-0.5">
            <a class="flex items-center px-2.5 py-2 rounded whitespace-nowrap" :class="isExactActive && 'bg-blue-50 dark:bg-gray-900'" :href="href" @click="navigate">
              <svg class="w-4 h-4 mr-2 fill-current shrink-0 text-slate-400 dark:text-white" :class="isExactActive && 'text-blue-400'" viewBox="0 0 16 16">
        ...
        <li v-if="!isMobile" class="mr-0.5 md:mr-0 md:mb-0.5">
          <a class="flex items-center px-2.5 py-2 rounded whitespace-nowrap cursor-pointer" @click="openPortal">
            <svg class="w-4 h-4 mr-2 fill-current shrink-0 text-slate-400 dark:text-white" viewBox="0 0 16 16">
        ...
        <router-link v-if="isUserAdmin" v-slot="{ href, navigate, isExactActive }" to="/dashboard/settings/admin" custom>
          <li class="mr-0.5 md:mr-0 md:mb-0.5">
            <a class="flex items-center px-2.5 py-2 rounded whitespace-nowrap" :class="isExactActive && 'bg-blue-50 dark:bg-gray-900'" :href="href" @click="navigate">
              <svg id="footer-sample-full" class="w-4 h-4 mr-2 fill-current shrink-0 text-slate-400 dark:text-white" :class="isExactActive && 'text-blue-400'" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" /><path stroke-linecap="round" d="M12 7v6m0 3.5v.5" /></g></svg>

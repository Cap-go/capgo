...
<template>
  <div class="relative overflow-x-auto pb-20 pb-4 shadow-md sm:rounded-lg md:pb-0">
    <div class="flex items-start justify-between pb-4 md:items-center">
      <div class="mb-2 flex md:mb-0">
        ...
        <button v-if="filterText && filterList.length" id="dropdownRadioButton" data-dropdown-offset-skidding="100" data-dropdown-toggle="dropdownRadio" class="relative inline-flex items-center border border-gray-300 rounded-lg bg-white px-3 py-1.5 text-sm font-medium text-gray-500 dark:border-gray-600 dark:bg-gray-800 hover:bg-gray-100 dark:text-white focus:outline-none focus:ring-4 focus:ring-gray-200 dark:hover:border-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-700" type="button">
          <div v-if="filterActivated" class="absolute h-6 w-6 inline-flex items-center justify-center border-2 border-white rounded-full bg-red-500 text-xs font-bold text-white -right-2 -top-2 dark:border-gray-900">
            {{ filterActivated }}
          </div>
          <IconFilter class="m-1 mr-2" />
          <span class="hidden md:block">{{ filterText }}</span>
          <IconDown class="m-1 ml-2 hidden md:block" />
        </button>
        <!-- Dropdown menu -->
        <div id="dropdownRadio" class="z-50 hidden w-48 border rounded-lg bg-white shadow divide-y divide-gray-100 dark:bg-gray-700 dark:divide-gray-600" data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="top">
          <div class="block px-4 py-3 text-sm text-gray-900 md:hidden dark:text-white">
            <div>{{ filterText }}</div>
          </div>
          <ul class="p-3 text-sm text-gray-700 space-y-1 dark:text-gray-200" aria-labelledby="dropdownRadioButton">
            <li v-for="(f, i) in filterList" :key="i">
              <div class="flex items-center rounded p-2 hover:bg-gray-100 dark:hover:bg-gray-600">
                <input :id="`filter-radio-example-${i}`" v-model="(filters as any)[f]" type="checkbox" :name="`filter-radio-${i}`" class="h-4 w-4 border-gray-300 bg-gray-100 text-blue-600 dark:border-gray-600 dark:bg-gray-700 focus:ring-2 focus:ring-blue-500 dark:ring-offset-gray-800 dark:focus:ring-blue-600 dark:focus:ring-offset-gray-800" @click="emit('filterClick', { clicked: f, filters })">
                <label :for="`filter-radio-example-${i}`" class="ml-2 w-full rounded text-sm font-medium text-gray-900 dark:text-gray-300">{{ f }}</label>
              </div>
            </li>
          </ul>
        </div>
      ...
      <div class="relative w-70 md:w-auto">
        ...
        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
          <IconSearch class="h-5 w-5 text-gray-500 dark:text-gray-400" />
        </div>
        <input id="table-search" v-model="search" type="text" class="block w-full border border-gray-300 rounded-lg bg-gray-50 p-2 pl-10 text-sm text-gray-900 md:w-80 dark:border-gray-600 focus:border-blue-500 dark:bg-gray-700 dark:text-white focus:ring-blue-500 dark:focus:border-blue-500 dark:focus:ring-blue-500 dark:placeholder-gray-400" :placeholder="searchPlaceholder">
      </div>
    </div>
      ...
      <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
        ...
        <tbody v-if="!isLoading">
          <tr
            ...
            class="border-b bg-white dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600"
            @click="emit('rowClick', elem)"
          >
            <template v-for="(col, _y) in columns" :key="`${i}_${_y}`">
              <th v-if="col.head" scope="row" class="whitespace-nowrap px-6 py-4 font-medium text-gray-900 dark:text-white">
                {{ displayValueKey(elem, col) }}
              </th>
              <td v-else-if="col.icon" :class="col.class" class="cursor-pointer px-6 py-4" @click.stop="col.onClick ? col.onClick(elem) : () => {}" v-html="col.icon" />
    ...
    <nav class="fixed bottom-0 left-0 z-40 w-full flex items-center justify-between bg-white p-4 md:relative dark:bg-gray-900 md:bg-transparent md:pt-4 dark:md:bg-transparent" aria-label="Table navigation">
      ...
      <ul class="inline-flex items-center -space-x-px">
        <li>
          <button class="ml-0 block border border-gray-300 rounded-l-lg bg-white px-3 py-2 leading-tight text-gray-500 dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-100 dark:text-gray-400 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-white" @click="fastBackward">
            <span class="sr-only">Fast Backward</span>
            <IconFastBackward />
          </button>
        </li>
        <li>
          <button class="ml-0 block border border-gray-300 bg-white px-3 py-2 leading-tight text-gray-500 dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-100 dark:text-gray-400 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-white" @click="prev">
            <span class="sr-only">Previous</span>
            <IconPrev />
          </button>
        </li>
        <li>
          <button aria-current="page" class="z-10 border border-blue-300 bg-blue-50 px-3 py-2 leading-tight text-blue-600 dark:border-gray-700 dark:bg-gray-700 hover:bg-blue-100 dark:text-white hover:text-blue-700">
            {{ currentPage }}
          </button>
        </li>
        <li>
          <button class="block border border-gray-300 bg-white px-3 py-2 leading-tight text-gray-500 dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-100 dark:text-gray-400 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-white" @click="next">
            <span class="sr-only">Next</span>
            <IconNext />
          </button>
        </li>
        <li>
          <button class="block border border-gray-300 rounded-r-lg bg-white px-3 py-2 leading-tight text-gray-500 dark:border-gray-700 dark:bg-gray-800 hover:bg-gray-100 dark:text-gray-400 hover:text-gray-700 dark:hover:bg-gray-700 dark:hover:text-white" @click="fastForward">

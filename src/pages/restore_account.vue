        ...
        handler: async () => {
          ...
          try {
            if (!pendingEmail.value || !pendingPassword.value) {
              ...
              return setErrors('restore-account', [t('invalid-auth')], {})
            }

            if (captchaKey.value && !confirmCaptchaToken.value) {
              isLoading.value = false
              return setErrors('restore-account', [t('captcha-required', 'Captcha verification is required')], {})
            }
            ...
            if (claimsError || !userId) {
              isLoading.value = false
              return setErrors('restore-account', [t('something-went-wrong-try-again-later')], {})
            }

            // Restore user using RPC function
...
async function submit(form: { email: string, password: string }) {
  ...
  if (captchaKey.value && !turnstileToken.value) {
    isLoading.value = false
    setErrors('restore-account', [t('captcha-required', 'Captcha verification is required')], {})
    return
  }
...
<template>
  ...
  <section class="flex overflow-y-auto py-10 my-auto w-full h-full sm:py-8 lg:py-2">
    <div class="px-4 my-auto mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="relative mx-auto mt-8 max-w-md md:mt-4">
        <div class="overflow-hidden bg-white rounded-md shadow-md dark:bg-slate-800">
          <div class="py-6 px-4 sm:py-7 sm:px-8">
            <FormKit id="restore-account" type="form" :actions="false" @submit="submit">
              <div class="space-y-5">
                <FormKit
                ...
                <div>
                  <div class="inline-flex justify-center items-center w-full">
                    ...
                    <button
                      ...
                      class="inline-flex justify-center items-center py-4 px-4 w-full text-base font-semibold text-white rounded-md transition-all duration-200 hover:bg-green-600 focus:bg-green-600 bg-green-500 focus:outline-hidden"
                    >
                      {{ t('restore-account') }}
                    </button>
                  </div>
      ...
      <Teleport v-if="dialogStore.showDialog && dialogStore.dialogOptions?.id === 'restore-account-confirm'" to="#dialog-v2-content" defer>

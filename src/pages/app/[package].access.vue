...
const { currentApp } = storeToRefs(mainStore)
...
async function checkRbacEnabled() {
  ...
  try {
    const { data, error } = await supabase
      .from('orgs')
      .select('use_new_rbac')
      .eq('id', currentApp.value.owner_org)
      .single()

    if (error)
      throw error

    useNewRbac.value = data?.use_new_rbac || false
  }
  catch (error: any) {
...
async function fetchAppRoleBindings() {
  if (!appId.value || !currentApp.value?.owner_org)
    return

  isLoading.value = true
  try {
    const { data, error } = await supabase.functions.invoke(`private/role_bindings/${currentApp.value.owner_org}`, {
      method: 'GET',
    })
    ...
    const appBindings = data.filter((b: RoleBinding) =>
      b.scope_type === 'app' && b.app_id === appId.value)
  ...
  try {
    const { data, error } = await supabase
      ...
      .select(`
        ...
        users!inner(email)
      `)
      .eq('org_id', currentApp.value.owner_org)
...
async function fetchAvailableGroups() {
  ...
  try {
    const { data, error } = await supabase.functions.invoke(`private/groups/${currentApp.value.owner_org}`, {
      method: 'GET',
    })
...
async function assignRole() {
  if (!isAssignRoleFormValid.value || !currentApp.value?.owner_org || !appId.value)
    return
  ...
  try {
    const { error } = await supabase.functions.invoke('private/role_bindings', {
      ...
      body: {
        ...
        scope_type: 'app',
        org_id: currentApp.value.owner_org,
        app_id: appId.value,
        channel_id: null,
...
async function removeRoleBinding(bindingId: string) {
  const confirmed = await dialogStore.openConfirmV2({
    title: t('remove-role'),
    message: t('remove-role-confirm'),
        ...
        <div class="form-control mt-4">
          ...
          <select v-model="assignRoleForm.principal_id" class="d-select" required>
            ...
            <option
              ...
              :key="assignRoleForm.principal_type === 'user' ? item.user_id : item.id"
              :value="assignRoleForm.principal_type === 'user' ? item.user_id : item.id"
            >
              {{ assignRoleForm.principal_type === 'user' ? item.email : item.name }}
            </option>
          </select>
        </div>

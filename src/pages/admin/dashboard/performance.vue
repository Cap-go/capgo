...
async function loadGlobalStatsTrend() {
  ...
  try {
    ...
    console.log('[Admin Dashboard Performance] Global stats trend data:', data)
    globalStatsTrendData.value = data || []
  }
  catch (error) {
    console.error('[Admin Dashboard Performance] Error loading global stats trend:', error)
...
onMounted(async () => {
  ...
  displayStore.NavTitle = t('performance')
})

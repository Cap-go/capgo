  ...
  <IonPage>
    <IonContent :fullscreen="true">
      <div class="grid place-content-center w-full h-full min-h-screen p-8">
        ...
        <h1 class="text-3xl text-left font-medium text-pumpkin-orange-500">
          {{ t('login.hello') }} !
        </h1>
        <h2 class="text-3xl text-left font-medium text-pumpkin-orange-500">
          {{ t('login.login-in') }}
        </h2>
        <p class="text-cornflower-500 text-1xl text-left ion-margin-vertical font-light leading-5 font-semibold">
        ...
        <form class="place-content-center w-full h-full" @submit.prevent="submit">
          ...
          <IonItem class="ion-no-padding">
            ...
            <IonInput id="passwordInput" v-model="form.password" autocomplete="current-password" name="password" enterkeyhint="send" :disabled="isLoading" :type="showPassword ? 'text' : 'password'" :placeholder="t('login.password') " :required="true" />

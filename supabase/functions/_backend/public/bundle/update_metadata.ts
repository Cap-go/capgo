...
app.post('/', middlewareKey(['all', 'write']), async (c) => {
  try {
    ...
    // Get the apikey from middleware
    const apikey = c.get('apikey') as any
    const { data: version, error: versionError } = await supabaseApikey(c as any, apikey.key)
      .from('app_versions')
      .select('*')
    ...
    const { error: updateError } = await supabaseApikey(c as any, apikey.key)
      .from('app_versions')
      .update(updateData)
      .eq('app_id', body.app_id)
      .eq('id', body.version_id)

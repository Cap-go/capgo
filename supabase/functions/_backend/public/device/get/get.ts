  ...
  app.openapi(route, async (c: Context) => {
    ...
    try {
      ...
      if (body.device_id) {
        ...
        const parsedResponse = validResponseSchema.safeParse(dataDevice)
        ...
        const parsedResponse = validResponseSchema.safeParse(dataDevices)
        if (!parsedResponse.success) {
          console.error('Database response does not match schema', parsedResponse.error)
          return c.json({ status: 'Database response does not match schema', error: parsedResponse.error }, 500)
        }
      ...
      console.error('Cannot get device', e)

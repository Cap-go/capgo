import { middlewareAPISecret, type MiddlewareKeyVariables } from '../utils/hono.ts'
import { Hono } from 'hono/tiny'
import { getEnv, backgroundTask } from '../utils/utils.ts'
import { getPgClient, closeClient } from '../utils/pg.ts'
...
function get_netlify_function_url(envGetters: any): string {
    ...
    if (!url) throw new Error('NETLIFY_FUNCTION_URL not set in environment');
    return url;
}
...
function get_apikey(envGetters: any): string {
    ...
    if (!key) throw new Error('APISECRET not set in environment');
    return key;
}
...
function get_cloudflare_function_url(envGetters: any): string {
    ...
    if (!url) throw new Error('CLOUDFLARE_FUNCTION_URL not set in environment');
    return url;
}

function get_cloudflare_pp_function_url(envGetters: any): string {
    const url = envGetters('CLOUDFLARE_PP_FUNCTION_URL');
    if (!url) throw new Error('CLOUDFLARE_PP_FUNCTION_URL not set in environment');
    return url;
}

function get_db_url(envGetters: any): string {
    const url = envGetters('DB_URL');
    if (!url) throw new Error('DB_URL not set in environment');
    return url;
}
    ...
    try {
        await backgroundTask(c as any, (async () => {
            ...
            try {
                ...
                if (sql) await closeClient(c as any, sql);

...
import { error, json, Router, StatusError } from 'itty-router'
...
import type { EnvTus } from './util.ts'
...
export class UploadHandler {
  ...
  constructor(state: DurableObjectState, env: EnvTus, bucket: R2Bucket) {
    this.state = state
    this.env = env
    this.parts = []
  ...
  async create(request: IRequest): Promise<Response> {
    ...
    if (r2Key == null) {
      console.log('bad filename metadata')
    ...
    if (Number.isNaN(uploadLength) && Number.isNaN(uploadDeferLength)) {
      console.log('must contain Upload-Length or Upload-Defer-Length header')
    ...
    if (!Number.isNaN(uploadDeferLength) && uploadDeferLength !== 1) {
      console.log('bad Upload-Defer-Length')
  ...
  async patch(request: IRequest): Promise<Response> {
    ...
    if (uploadInfo.uploadLength != null && !Number.isNaN(headerUploadLength) && uploadInfo.uploadLength !== headerUploadLength) {
      console.log('upload length cannot change')
    ...
    if (request.body == null) {
      console.log('Must provide request body')
...
export class AttachmentUploadHandler extends UploadHandler {
  constructor(state: DurableObjectState, env: EnvTus) {

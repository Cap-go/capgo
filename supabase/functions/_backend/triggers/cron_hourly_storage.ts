...
const inputBodySchema = z.object({
    app_id: z.string(),
    current_hour: z.iso.datetime().optional()
})
...
const cacheSchema = z.object({
    storageChanges: z.array(z.object({
        ...
        storage_added: z.iso.datetime().optional(),
        storage_removed: z.iso.datetime().optional(),
...
app.post('/', middlewareAPISecret, async (c) => {
    try {
        ...
        const { app_id, current_hour } = inputBodySchema.parse(body)

        const executionNow = current_hour ? dayjs(current_hour) : dayjs()

...
app.post('/grant', middlewareV2(), async (c) => {
...
app.get('/search-orgs', middlewareV2(), async (c) => {
...
app.get('/org-balance/:orgId', middlewareV2(), async (c) => {
  const { isAdmin } = await verifyAdmin(c)
...
app.get('/grants-history', middlewareV2(), async (c) => {
  const { isAdmin } = await verifyAdmin(c)

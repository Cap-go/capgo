...
export async function updateWithPG(c: Context, body: AppInfos, drizzleCient: ReturnType<typeof getDrizzleClient> | ReturnType<typeof getDrizzleClientD1>, isV2: boolean) {
  try {
    ...
    if (!channelData && !channelOverride) {
      console.log({ requestId: c.get('requestId'), message: 'Cannot get channel or override', id: app_id, date: new Date().toISOString() })
      if (versionData)
    ...
    if (!version.external_url && !version.r2_path && version.name !== 'builtin' && (!manifestEntries || manifestEntries.length === 0)) {
      console.log({ requestId: c.get('requestId'), message: 'Cannot get bundle', id: app_id, version, manifestEntriesLength: manifestEntries ? manifestEntries.length : 0, channelData: channelData ? channelData.channels.name : 'no channel data', defaultChannel })
    ...
    // console.log(c.get('requestId'), 'check disableAutoUpdateToMajor', device_id)
      if (!channelData.channels.ios && platform === 'ios') {
        console.log({ requestId: c.get('requestId'), message: 'Cannot update, ios is disabled', id: device_id, date: new Date().toISOString() })
      ...
      if (!channelData.channels.android && platform === 'android') {
        console.log({ requestId: c.get('requestId'), message: 'Cannot update, android is disabled', id: device_id, date: new Date().toISOString() })
        await sendStatsAndDevice(c, device, [{ action: 'disablePlatformAndroid' }])
        console.log({ requestId: c.get('requestId'), message: 'sendStats', date: new Date().toISOString() })
        return c.json({
      ...
      if (version.name !== 'builtin' && channelData?.channels.disable_auto_update === 'major' && parse(version.name).major > parse(version_build).major) {
        console.log({ requestId: c.get('requestId'), message: 'Cannot upgrade major version', id: device_id, date: new Date().toISOString() })
      ...
      if (!channelData.channels.allow_device_self_set && !channelData.channels.public) {
        console.log({ requestId: c.get('requestId'), message: 'Cannot update via a private channel', id: device_id, date: new Date().toISOString() })
      ...
      if (version.name !== 'builtin' && channelData.channels.disable_auto_update === 'minor' && parse(version.name).minor > parse(version_build).minor) {
        console.log({ requestId: c.get('requestId'), message: 'Cannot upgrade minor version', id: device_id, date: new Date().toISOString() })
      ...
      console.log({ requestId: c.get('requestId'), message: 'version', version: version.name, old: version_name })
      if (version.name !== 'builtin' && channelData.channels.disable_auto_update === 'patch' && !(
        ...
        console.log({ requestId: c.get('requestId'), message: 'Cannot upgrade patch version', id: device_id, date: new Date().toISOString() })
      ...
      if (version.name !== 'builtin' && channelData.channels.disable_auto_update === 'version_number') {
        ...
        if (minUpdateVersion === null) {
          console.log({ requestId: c.get('requestId'), message: 'Channel is misconfigured', channel: channelData.channels.name, date: new Date().toISOString() })
        ...
        if (greaterThan(parse(minUpdateVersion), parse(version_build))) {
          console.log({ requestId: c.get('requestId'), message: 'Cannot upgrade, metadata > current version', id: device_id, min: minUpdateVersion, old: version_name, date: new Date().toISOString() })
      ...
      if (version.name !== 'builtin' && channelData.channels.disable_auto_update_under_native && lessThan(parse(version.name), parse(version_build))) {
        console.log({ requestId: c.get('requestId'), message: 'Cannot revert under native version', id: device_id, date: new Date().toISOString() })
      ...
      if (!channelData.channels.allow_dev && !is_prod) {
        console.log({ requestId: c.get('requestId'), message: 'Cannot update dev build is disabled', id: device_id, date: new Date().toISOString() })
      ...
      if (!channelData.channels.allow_emulator && is_emulator) {
        console.log({ requestId: c.get('requestId'), message: 'Cannot update emulator is disabled', id: device_id, date: new Date().toISOString() })
    ...
    console.log({ requestId: c.get('requestId'), message: 'bundle_url_timing', duration: `${endBundleUrl - startBundleUrl}ms`, date: new Date().toISOString() })

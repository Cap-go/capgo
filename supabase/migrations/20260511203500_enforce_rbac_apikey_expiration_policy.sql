...
BEGIN
  ...
  IF p_apikey IS NOT NULL THEN
    ...
    IF COALESCE(v_org_requires_apikey_expiration, false) AND v_api_key.expires_at IS NULL THEN
      PERFORM public.pg_log('deny: RBAC_CHECK_PERM_APIKEY_EXPIRATION_REQUIRED', jsonb_build_object(
        'permission', p_permission_key,
        'key_id', v_api_key.id,
        'org_id', v_effective_org_id,
        'app_id', v_effective_app_id,

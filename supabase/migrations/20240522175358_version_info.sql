...
BEGIN
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    LEFT JOIN channels ch ON NEW.app_id = ch.app_id
    LEFT JOIN channel_devices cd ON d.device_id = cd.device_id AND cd.channel_id = ch.id
    WHERE (d.device_id IS NOT NULL OR ch.public = TRUE)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
      ...
      SELECT 
        av.app_id, cd.device_id, av.id, av.name, av.checksum, av.session_key, av.bucket_id, 
        av.storage_provider, av.external_url, av.r2_path, av."minUpdateVersion", NEW.id, NEW.name,
        NEW.allow_dev, NEW.allow_emulator, NEW."disableAutoUpdateUnderNative" AS disable_auto_update_under_native,
        NEW."disableAutoUpdate" AS disable_auto_update,
        NEW.ios, NEW.android, NEW."secondaryVersionPercentage" AS secondary_version_percentage, NEW.enable_progressive_deploy,
        NEW."enableAbTesting" AS enable_ab_testing, NEW.allow_device_self_set, NEW.public, CURRENT_TIMESTAMP
      FROM app_versions av
      LEFT JOIN channel_devices cd ON av.app_id = cd.app_id AND cd.channel_id = NEW.id
  ...
  -- Delete
  IF TG_OP = 'DELETE' THEN
    DELETE FROM version_info WHERE version_info.channel_id = OLD.id;
    RETURN OLD;
  END IF;
...
BEGIN
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    ...
    SELECT 
      NEW.app_id, NEW.device_id, av.id, av.name, av.checksum, av.session_key, av.bucket_id, 
      av.storage_provider, av.external_url, av.r2_path, av."minUpdateVersion", ch.id, ch.name,
      ...
      ch."enableAbTesting" AS enable_ab_testing, ch.allow_device_self_set, ch.public, CURRENT_TIMESTAMP
    FROM app_versions av
    LEFT JOIN channel_devices cd ON NEW.device_id = cd.device_id  -- Ensure 'cd' is joined correctly
    LEFT JOIN channels ch ON cd.channel_id = ch.id
  ...
  IF TG_OP = 'DELETE' THEN
    DELETE FROM version_info WHERE app_id = OLD.app_id AND device_id = OLD.device_id AND version_id = OLD.version AND channel_id = OLD.channel_id;
    RETURN OLD;
  END IF;

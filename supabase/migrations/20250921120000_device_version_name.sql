...
WHERE av.id = d.version
  AND (d.version_name IS NULL OR d.version_name = '');

UPDATE public.devices
SET version_name = COALESCE(NULLIF(version_name, ''), 'unknown')
WHERE version_name IS NULL OR version_name = '';

ALTER TABLE public.devices
  ALTER COLUMN version_name SET DEFAULT 'unknown';
...
SET version_name = av.name
FROM public.app_versions av
WHERE av.id = s.version
  AND (s.version_name IS NULL OR s.version_name = '');

-- Activation tracking: timestamp of the first device-delivered update per app.
-- Claimed idempotently by the daily logsnag_insights cron, which emits the
-- 'First Device Update Delivered' analytics event for fresh activations.
ALTER TABLE "public"."apps" ADD COLUMN IF NOT EXISTS "first_update_delivered_at" timestamp with time zone;

-- get_org_apps_with_last_upload returns the full apps row via `a.*`, so its
-- declared RETURNS TABLE must match the new physical column order. Changing a
-- function's return columns requires DROP + recreate.
DROP FUNCTION IF EXISTS "public"."get_org_apps_with_last_upload"(

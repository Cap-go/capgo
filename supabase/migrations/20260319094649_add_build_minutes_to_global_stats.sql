...
WITH daily_build_stats AS (
    SELECT
        ...
        COALESCE(
            SUM(CASE WHEN platform = 'ios' THEN build_time_unit ELSE 0 END), 0
        )
        / 60.0 AS ios_minutes,
        COALESCE(
            SUM(CASE WHEN platform = 'android' THEN build_time_unit ELSE 0 END),
            0
        )
        / 60.0 AS android_minutes,
...
SET
    build_minutes_day_ios = daily_build_stats.ios_minutes,
    build_minutes_day_android = daily_build_stats.android_minutes,
    builds_day_ios = daily_build_stats.ios_builds,
    builds_day_android = daily_build_stats.android_builds

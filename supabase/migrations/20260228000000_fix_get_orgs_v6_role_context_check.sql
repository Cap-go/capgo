-- Align get_orgs_v6(userid uuid) authorization check with runtime role context
-- and existing test/auth flows.
...
BEGIN
  ...
  IF caller_role IS DISTINCT FROM 'service_role' AND caller_role IS DISTINCT FROM 'postgres' THEN
    IF caller_user_id IS DISTINCT FROM userid THEN
      PERFORM public.pg_log(
        'deny: GET_ORGS_V6_UNAUTHORIZED_USER',
        jsonb_build_object('requested_user_id', userid, 'resolved_user_id', caller_user_id, 'role', caller_role)
      );

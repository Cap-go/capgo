...
SELECT app_id, manifest_bundle_count AS base_count
FROM public.apps
WHERE app_id = 'com.demo.app';

SELECT
    is(
        (SELECT COUNT(*)::bigint FROM tmp_manifest_app),
        1::bigint,
...
SELECT
    is(
        (SELECT base_count FROM tmp_manifest_app),
        (
          SELECT COUNT(DISTINCT av.id)::bigint
          FROM public.app_versions av
          WHERE av.app_id = (SELECT app_id FROM tmp_manifest_app)
            AND EXISTS (
              SELECT 1
...
WITH new_version AS (
  ...
  SELECT
    app_id,
    concat('manifest-queue-test-', floor(extract(epoch FROM clock_timestamp()) * 1000)::text),
    owner_org
  FROM public.apps
  WHERE app_id = (SELECT app_id FROM tmp_manifest_app)
  LIMIT 1
  RETURNING id
)
...
SELECT
    ok(
        (SELECT version_id IS NOT NULL FROM tmp_manifest_version),
        'Test version inserted'
    );

SELECT
    is(
        (SELECT manifest_bundle_count FROM public.apps WHERE app_id = (SELECT app_id FROM tmp_manifest_app)),
        (SELECT base_count FROM tmp_manifest_app),
...
WITH inserted AS (
  ...
  SELECT version_id, 'manifest-first.js', '/tests/manifest-first.js', 'hash-first'
  FROM tmp_manifest_version
  RETURNING id, 'first'::text AS label
)
INSERT INTO tmp_manifest_rows
SELECT id, label
FROM inserted;
...
SELECT
    ok(
        EXISTS (
            SELECT 1
            FROM pgmq.q_manifest_bundle_counts
            WHERE (message ->> 'app_id') = (SELECT app_id FROM tmp_manifest_app)
              AND (message ->> 'delta')::integer = 1
              AND (message ->> 'app_version_id')::bigint = (SELECT version_id FROM tmp_manifest_version)
        ),
...
SELECT
    is(
        (SELECT manifest_bundle_count FROM public.apps WHERE app_id = (SELECT app_id FROM tmp_manifest_app)),
        (SELECT base_count + 1 FROM tmp_manifest_app),
        'Counter increments after processing'
    );

WITH inserted AS (
  INSERT INTO public.manifest (app_version_id, file_name, s3_path, file_hash)
  SELECT version_id, 'manifest-second.js', '/tests/manifest-second.js', 'hash-second'
  FROM tmp_manifest_version
  RETURNING id, 'second'::text AS label
)
INSERT INTO tmp_manifest_rows
SELECT id, label
FROM inserted;
...
SELECT
    is(
        (
          SELECT COUNT(*)::bigint
          FROM pgmq.q_manifest_bundle_counts
          WHERE (message ->> 'app_version_id')::bigint = (SELECT version_id FROM tmp_manifest_version)
        ),
        0::bigint,

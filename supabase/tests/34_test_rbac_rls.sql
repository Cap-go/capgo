...
-- 1) Admin cannot read rbac_settings
...
SELECT
    ok(
        (
            SELECT count(*)
            FROM
                public.rbac_settings
        ) = 0,
        'Admin cannot read rbac_settings'
    );

-- 2) Regular user cannot read rbac_settings
...
SELECT
    ok(
        ...
        'Regular user cannot read rbac_settings'
    );
...
-- 7) Admin can see role_bindings for their org
SELECT tests.authenticate_as('test_admin');
...
-- 9) Test admin cannot insert rbac_settings
SELECT tests.authenticate_as('test_admin');

SELECT
    throws_ok(
        $$INSERT INTO public.rbac_settings (id, use_new_rbac) VALUES (999, true)$$,
        '42501',
        'new row violates row-level security policy for table "rbac_settings"',
        'Admin cannot insert rbac_settings'
    );

-- 10) Test admin cannot create roles
SELECT tests.authenticate_as('test_admin');

...
SELECT is(tests.get_supabase_uid('test_admin'), 'c591b04e-cf29-4945-b9a0-776d0672061a', 'test get_supabase_uid - test_admin');
SELECT is(tests.get_supabase_uid('test_user'), '6aa76066-55ef-4238-ade6-0b32334a4097', 'test get_supabase_uid - test_user1');
SELECT is(tests.get_supabase_uid('test_user2'), '6f0d1a2e-59ed-4769-b9d7-4d9615b28fe5', 'test get_supabase_uid - test_user2');
SELECT tests.authenticate_as('test_user2');
SELECT is(verify_mfa(), true, 'test verify_mfa - test_user2');
SELECT is(coalesce(
...
SELECT is(invite_user_to_org('test3@capgo.app', (SELECT id FROM orgs WHERE created_by = tests.get_supabase_uid('test_admin')), 'read'), 'NO_EMAIL', 'invite_user_to_org test - no email');
SELECT tests.clear_authentication();

-- Test 2: Check if the function returns 'INVALID_ROLE' when the user_right is not an invite role
SELECT tests.authenticate_as('test_admin');
select is(invite_user_to_org('test@capgo.app', (SELECT id FROM orgs WHERE created_by = tests.get_supabase_uid('test_admin')), 'read'), 'OK', 'invite_user_to_org test - valid input');
SELECT tests.clear_authentication();
...
SELECT tests.authenticate_as('test_user');
SELECT is(accept_invitation_to_org('22dbad8a-b885-4309-9b3b-a09f8460fb6d'), 'OK', 'accept_invitation_to_org test - valid input');

SELECT is(
  (SELECT user_right FROM org_users WHERE user_id = tests.get_supabase_uid('test_user') AND org_id = '22dbad8a-b885-4309-9b3b-a09f8460fb6d'),

...
INSERT INTO public.apps (app_id, icon_url, owner_org, name, retention, default_upload_channel)
SELECT
  app_id,
  '',
  org_id,
...
WITH ios_version AS (
  ...
  SELECT app_id, '1.0.0-ios', org_id
  FROM deploy_stats_context
  RETURNING id
),
android_version AS (
  INSERT INTO public.app_versions (app_id, name, owner_org)
  SELECT app_id, '1.0.0-android', org_id
  FROM deploy_stats_context
  RETURNING id
)
...
WITH ios_channel AS (
  INSERT INTO public.channels (created_by, app_id, name, version, public, ios, android, owner_org)
  SELECT user_id, app_id, 'prod-ios', ios_version_id, true, true, false, org_id
  FROM deploy_stats_context
  RETURNING id
),
android_channel AS (
  INSERT INTO public.channels (created_by, app_id, name, version, public, ios, android, owner_org)
  SELECT user_id, app_id, 'prod-android', android_version_id, true, false, true, org_id
  FROM deploy_stats_context
  RETURNING id
),
private_channel AS (
  INSERT INTO public.channels (created_by, app_id, name, version, public, ios, android, owner_org)
  SELECT user_id, app_id, 'private-beta', ios_version_id, false, true, false, org_id
  FROM deploy_stats_context
  RETURNING id
)
UPDATE deploy_stats_context
...
SELECT
  is(
    (
      ...
      WHERE message -> 'payload' ->> 'appId' = (SELECT app_id FROM deploy_stats_context)
        AND message -> 'payload' ->> 'type' = 'deploy_install_stats'
    ),
    2::bigint,

...
SELECT
    is(
        (SELECT COUNT(*) FROM tmp_channel_device_counts),
        1::bigint,
...
SELECT
    is(
        (SELECT base_count FROM tmp_channel_device_counts),
        (SELECT COUNT(*)::bigint FROM public.channel_devices WHERE app_id = 'com.demo.app'),
...
SELECT id, app_id, 'queue-test-device', owner_org
...
SELECT
    is(
        (SELECT channel_device_count FROM public.apps WHERE app_id = 'com.demo.app'),
        (SELECT base_count FROM tmp_channel_device_counts),
...
SELECT
    ok(
        EXISTS (
            SELECT 1
            FROM pgmq.q_channel_device_counts
            WHERE message ->> 'device_id' = 'queue-test-device'
...
SELECT
    is(
        (SELECT channel_device_count FROM public.apps WHERE app_id = 'com.demo.app'),
        (SELECT base_count + 1 FROM tmp_channel_device_counts),

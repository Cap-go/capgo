...
CREATE POLICY "Allow update for auth and api keys" ON "public"."app_versions" FOR UPDATE TO "anon", "authenticated" USING ((EXISTS ( SELECT 1
  ...
  WHERE ((("identity"."uid" IS NOT NULL) AND "public"."check_min_rights"('write'::"public"."user_min_right", "identity"."uid", "app_versions"."owner_org", "app_versions"."app_id", NULL::bigint)) OR (("identity"."uid" IS NULL) AND ("identity"."apikey" IS NOT NULL) AND "public"."check_min_rights"('upload'::"public"."user_min_right", NULL::"uuid", "app_versions"."owner_org", "app_versions"."app_id", NULL::bigint)))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM ( SELECT "auth"."uid"() AS "uid",
            "public"."get_apikey_header"() AS "apikey") "identity"
  WHERE ((("identity"."uid" IS NOT NULL) AND "public"."check_min_rights"('write'::"public"."user_min_right", "identity"."uid", "app_versions"."owner_org", "app_versions"."app_id", NULL::bigint)) OR (("identity"."uid" IS NULL) AND ("identity"."apikey" IS NOT NULL) AND "public"."check_min_rights"('upload'::"public"."user_min_right", NULL::"uuid", "app_versions"."owner_org", "app_versions"."app_id", NULL::bigint))))));

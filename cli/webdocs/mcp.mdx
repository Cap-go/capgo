...
Capacitor continues to load the active root config, while every MCP tool that writes Capacitor config updates only the supplied app source.
The live-update onboarding start tool also accepts `capacitorConfig` when the server was launched without this option.

...
- **Does Capgo Builder's publish do the first upload as a draft? (Capgo-side check.)** Counter-evidence (fastlane #18293) shows the *first* upload for a freshly-created (never-released) package works via the API only with `release_status: "draft"`. If our builder's publish path doesn't set draft for a never-released package, the first build would fail even after the user creates the app — making Path B feel broken. Verify what our builder sends; if needed, have it use draft on first publish. This determines whether Path B truly "completes after one click."

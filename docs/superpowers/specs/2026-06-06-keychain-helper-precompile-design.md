...
| Pipeline | Dual-trigger GitHub Actions workflow on `macos-latest`: build → codesign → notarize → verify → npm publish with provenance; the run creates the `cli-helper-X.Y.Z` git tag + GitHub release (via the built-in `GITHUB_TOKEN`, which is loop-safe). Triggers: the deliberate **`workflow_dispatch`** button (version input) for normal releases, **and** a pushed `cli-helper-X.Y.Z` tag — the tag path lets the whole pipeline be tested from a PR branch without merging. Intentionally separate from the repo's auto-tag `bump_version.yml` path used by `capgo`/`cli` |

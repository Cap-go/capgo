  ...
  describe('rbac_check_permission_direct SQL function', () => {
    ...
    describe('rbac_is_enabled_for_org', () => {
      it('should return true when global flag is enabled', async () => {
        await pool.query(`UPDATE public.rbac_settings SET use_new_rbac = true WHERE id = 1`)

        const result = await pool.query(`
          SELECT public.rbac_is_enabled_for_org($1::uuid) as enabled
        `, [ORG_ID])

        await pool.query(`UPDATE public.rbac_settings SET use_new_rbac = false WHERE id = 1`)

        expect(result.rows[0].enabled).toBe(true)
      })

      it('should return true when org flag is enabled', async () => {
        await pool.query(`UPDATE public.orgs SET use_new_rbac = true WHERE id = $1`, [ORG_ID])

        const result = await pool.query(`
          SELECT public.rbac_is_enabled_for_org($1::uuid) as enabled
        `, [ORG_ID])

        await pool.query(`UPDATE public.orgs SET use_new_rbac = false WHERE id = $1`, [ORG_ID])

...
describe('app_versions RLS DoS regression', () => {
  it.concurrent('returns an empty response for unauthenticated unfiltered reads', async () => {
    const response = await fetch(`${SUPABASE_BASE_URL}/rest/v1/app_versions?select=id&limit=1`, {
  ...
  it.concurrent('keeps unrelated anonymous reads available during parallel app_versions probes', async () => {
    ...
    const responses = await Promise.all(Array.from({ length: 8 }, () => fetch(`${SUPABASE_BASE_URL}/rest/v1/app_versions?select=id&limit=1`, { headers })))
    const bodies = await Promise.all(responses.map(response => response.text()))

    for (const [index, response] of responses.entries())
      expect(response.status, bodies[index]).toBe(200)

    const response = await fetch(`${SUPABASE_BASE_URL}/rest/v1/orgs?select=id&limit=1`, { headers })
    const body = await response.text()
  ...
  it.concurrent('keeps Capgo API key reads on app_versions working', async () => {
    const response = await fetch(`${SUPABASE_BASE_URL}/rest/v1/app_versions?select=id,app_id&app_id=eq.${APP_NAME}&limit=1`, {

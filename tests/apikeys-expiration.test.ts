...
describe('[PUT] /apikey/:id with expiration', () => {
  ...
  beforeAll(async () => {
    ...
    const response = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-for-update-expiration-${id}`,
        mode: 'all',
      }),
    })
    expect(response.status).toBe(200)
...
describe('[GET] /apikey with expiration info', () => {
  ...
  beforeAll(async () => {
    ...
    const response2 = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-without-exp-get-test-${id}`,
        mode: 'all',
      }),
    })
...
describe('organization API key expiration policy', () => {
  ...
  it('fail to create api key with expiration exceeding org max days', async () => {
    ...
    const response = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-policy-exceeds-max-${id}`,
        mode: 'all',
        limited_to_orgs: [POLICY_ORG_ID],
  ...
  it('fail to create app-scoped api key with expiration exceeding org max days', async () => {
    ...
    const response = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-policy-app-scope-too-far-${id}`,
        mode: 'all',
  ...
  it('fail to create app-scoped api key for unknown app ids', async () => {
    const response = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-policy-app-scope-missing-app-${id}`,
        mode: 'all',
  ...
  it('fail to update api key scope to app owner org without expiration (scope-change regression)', async () => {
    const createResponse = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-policy-app-scope-update-${id}`,
        mode: 'all',
      }),
    })
  ...
  it('reject limited app-scoped api keys from updating sibling keys', async () => {
    ...
    const limitedKeyResponse = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-policy-limited-updater-${id}`,
        mode: 'all',
        limited_to_apps: [POLICY_APPNAME],
        expires_at: validDate,
      }),
    ...
    const siblingKeyResponse = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-policy-sibling-target-${id}`,
        mode: 'all',
      }),
    })
    ...
    const updateResponse = await apiFetch(`/apikey/${siblingKey.id}`, {
      ...
      body: JSON.stringify({
        name: `key-policy-escalation-attempt-${id}`,
      }),
    })
    const data = await updateResponse.json() as { error: string }
  ...
  it('reject direct apikey inserts that bypass app-scoped expiration policy', async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        mode: 'all',
        name: `direct-insert-policy-bypass-${id}`,
        limited_to_apps: [POLICY_APPNAME],
        limited_to_orgs: [],
        expires_at: null,
      })
  ...
  it('reject hashed apikey RPC creation that exceeds app owner org max expiration', async () => {
    ...
    const { data, error } = await supabase.rpc('create_hashed_apikey', {
      p_mode: 'all',
      p_name: `direct-rpc-policy-bypass-${id}`,
      p_limited_to_orgs: [],
...
describe('api key expiration boundary conditions', () => {
  ...
  it('api key with null expiration should never expire', async () => {
    ...
    const response = await apiFetch('/apikey', {
      ...
      headers: authHeaders,
      body: JSON.stringify({
        name: `key-no-expiration-test-${id}`,
        mode: 'all',
      }),
    })

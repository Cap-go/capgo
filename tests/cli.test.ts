...
describe('test key generation', () => {
  const APPNAME = 'com.demo.app.cli_1'
  beforeEach(async () => {
  ...
  // set the key to an empty string. Otherwise runCli will not work
    ...
    const publicKeyFinalPath = join(tempFileFolder, publicKeyPath!)
    ...
    const privateKeyFinalPath = join(tempFileFolder, publicKeyPath!).replace('.pub', '')
  ...
  it('test new key generation', async () => {
    ...
    const privateKeyFinalPath = join(tempFileFolder, privateKeyPath!)
    expect(existsSync(privateKeyFinalPath!)).toBe(true)
...
describe('tests CLI upload', () => {
  const APPNAME = 'com.demo.app.cli_2'
  beforeAll(async () => {
    await resetAndSeedAppData(APPNAME)
    await prepareCli(APPNAME)
  })
  ...
  it('test custom key upload and download (old)', async () => {
    ...
    const privateKey = readFileSync(join(tempFileFolder, '.capgo_key'), 'utf-8')
    ...
    // test with key data
    const publicKeyFile = readFileSync(join(tempFileFolder, '.capgo_key.pub'), 'utf-8')
    expect(publicKeyFile).toContain('PUBLIC KEY')

    renameSync(join(tempFileFolder, '.capgo_key.pub'), join(tempFileFolder, 'wierd_file'))
    rmSync(join(tempFileFolder, '.capgo_key'))
  ...
  it('test custom key upload and download (new)', async () => {
    ...
    const publicKeyFile = readFileSync(join(tempFileFolder, '.capgo_key_v2.pub'), 'utf-8')
    ...
    // test with key data
    const privateKeyFile = readFileSync(join(tempFileFolder, '.capgo_key_v2'), 'utf-8')
    expect(privateKeyFile).toContain('PRIVATE KEY')

    renameSync(join(tempFileFolder, '.capgo_key_v2'), join(tempFileFolder, 'wierd_file'))
    rmSync(join(tempFileFolder, '.capgo_key_v2.pub'))
  ...
  it('test code check (missing index.html)', async () => {
    rmSync(join(tempFileFolder, 'dist', 'index.html'))
    const output = await runCli(['bundle', 'upload', '-b', semver, '-c', 'production', '--ignore-metadata-check'], false)
...
describe('tests Wrong cases', () => {
  const APPNAME = 'com.demo.app.cli_3'
  beforeAll(async () => {
    await resetAndSeedAppData(APPNAME)
    await prepareCli(APPNAME)
  })
...
describe('tests CLI metadata', () => {
  const APPNAME = 'com.demo.app.cli_4'
  beforeAll(async () => {
    await prepareCli(APPNAME)
  })

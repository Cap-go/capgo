...
async function createAuthUserWithRetry(
    ...
    for (let i = 0; i < maxRetries; i++) {
        const result = await getSupabaseClient().auth.admin.createUser({
            email,
            email_confirm: true,
        ...
        // If it's a 503, retry; otherwise return the error
...
describe('Organization Email Domain Auto-Join', () => {
    ...
    describe('Auto-Join Functionality', () => {
        it.skip('should auto-join user to org on signup with matching email domain', async () => {
            ...
            const { data: authUser, error: authError } = await createAuthUserWithRetry(
                testEmail,
                {
                    first_name: 'Test',
                    last_name: 'User',

...
describe('files app-scoped read availability', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
  })

  it('serves cached app-scoped files without checking app deletion state', async () => {
  ...
  it('serves cached malformed app-scoped paths without database reads', async () => {
    const bucketPut = vi.fn()
    globalThis.caches = {
      default: {

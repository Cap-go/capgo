...
async function clearCronStatAppMessages(appId: string) {
  await executeSQL(
    `DELETE FROM pgmq.q_cron_stat_app WHERE message->'payload'->>'appId' = $1`,
    [appId],
  );
}
...
describe("cron_stat_app queue resilience", () => {
  ...
  it.concurrent(
    "process_cron_stats_jobs still queues active apps when daily_mau is quiet",
    async () => {
      await clearCronStatAppMessages(processCronAppId);

      await executeSQL(`DELETE FROM public.daily_mau WHERE app_id = $1`, [
        processCronAppId,
      ]);
      await executeSQL(
      ...
      expect(queuedMessages[0]?.message?.payload?.orgId).toBe(
        ORG_ID_CRON_QUEUE,
      );
  ...
  it.concurrent(
    "live /stats traffic does not enqueue cron_stat_app directly",
    async () => {
      await clearCronStatAppMessages(liveQueueAppId);
